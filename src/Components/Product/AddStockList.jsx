  ...
  const fetchAddStocks = async () => {
    ...
    try {
      ...
      if (res.data.status === "Success") {
        ...
        ErrorToast("Failed to fetch AddStocks");
      }
    } catch (err) {
      console.error(err);
      ErrorToast("Something went wrong while fetching AddStocks");
    } finally {
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Showing {AddStocks.length} of {total} AddStocks
          </p>
        </div>
          ...
          <div className=" overflow-auto">
            <table className="global_table w-full" ref={printRef}>
              ...
              <tbody className="global_tbody">
                {AddStocks.map((AddStock) => (
                  <tr className="global_tr" key={AddStock._id}>
                    <td className="global_td">{AddStock.no}</td>
                    <td className="global_td">
                      {AddStock.Users?.[0]?.name || ""}
                    </td>

                    <td className="global_td min-w-[100px]">
                      <span className="mr-2">
                        {(() => {
                          const d = new Date(AddStock.CreatedDate);
                      ...
                      <TimeAgo date={AddStock.CreatedDate} />
                    </td>
                    <td className="global_td" id="no-print">
                      <Link
                        to={`/AddStockDetails/${AddStock._id}`}
